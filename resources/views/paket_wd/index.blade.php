...
<ol class="breadcrumb float-sm-right">
    ...
    <li class="breadcrumb-item"><a href="#">Paket WD</a></li>
...
@section('content')
	...
	<div class="card mb-12 border-left-primary">
	    <div class="card-body">
			...
			<a class="btn btn-success w-md m-b-5" href="{{url('paket_wd/create')}}"><i class="fa fa-plus"></i> Tambah Data</a>
	    </div>
	</div>
	...
	<div class="card card-default" id="main-box" style="">
        ...
        <div class="card-body">
			<table  id="tb_paket_wd" class="table table-bordered table-striped table-hover">
		    	<thead>
			        <tr>
			            <th width="5%">No.</th>
			            <th width="65%">Nama</th>
...
<script type="text/javascript">
	...
	var tb_paket_wd = $('#tb_paket_wd').DataTable( {
		paging:true,
		destroy: true,
        autoWidth: false,
        processing: true,
        serverSide: true,
        ajax: {
            "url" : '{{url("paket_wd/list_data_paket_wd")}}',
            "type" : "GET",
            "data" : function (d) {
            }
        },
		...
		function(){
			$.ajax({
				...
				success:  function(data){
					if(data==1){
						swal("Deleted!", "Data paket wd berhasil dihapus.", "success");
					}else{
						
						swal("Failed!", "Gagal menghapus data paket wd.", "error");
					}
				},
				complete: function(data){
					tb_paket_wd.draw(false);
				},
				error: function(data) {
	...
	function submit_valid(id){
		if($(".validated_form").valid()) {
			...
			$.ajax({
				...
				success:  function(data){
					if(data.status ==1){
						show_info("Data paket berhasil disimpan!");
	...
	function edit_data(id){
      	$.ajax({
	        ...
	        beforeSend: function(data){
		        ...
		        $("#modal-xl .modal-title").html("Edit Data - Paket WT");
		        $('#modal-xl').modal("show");
