...
@section('content')
	...
	<div class="card card-info card-outline mb-12 border-left-primary">
	    <div class="card-body">
			...
			<a class="btn btn-success w-md m-b-5" href="{{url('sub_kode_akuntansi/create')}}"><i class="fa fa-plus"></i> Tambah Data</a>
	    </div>
	</div>
  		...
  		<div class="card-header">
        	<h3 class="card-title">
          		<i class="fas fa-list"></i>
   				List Data Sub Kode Akuntansi
        	</h3>
      	</div>
        <div class="card-body">
			<table  id="tb_sub_kode_akuntansi" class="table table-bordered table-striped table-hover">
		    	<thead>
			        <tr>
			            <th width="5%">No.</th>
			            <th width="5%">Kode Akun</th>
			            <th width="35%">Nama Akun</th>
			            <th width="10%">Sub Kode Akun</th>
			            <th width="35%">Nama Sub Akun</th>
...
<script type="text/javascript">
	...
	var tb_sub_kode_akuntansi = $('#tb_sub_kode_akuntansi').dataTable( {
			processing: true,
	        serverSide: true,
	        stateSave: true,
	        ajax:{
			        url: '{{url("sub_kode_akuntansi/list_sub_kode_akuntansi")}}',
			        data:function(d){
				         }
			     },
	        columns: [
	            {data: 'no', name: 'no',width:"2%", class : 'text-center'},
	            {data: 'id_kode_akun', name: 'id_kode_akun', class : 'text-center'},
	...
	function delete_sub_kode_akuntansi(id){
		swal({
		...
		function(){
			$.ajax({
				...
				complete: function(data){
					tb_sub_kode_akuntansi.fnDraw(false);
				},
				error: function(data) {
	...
	function submit_valid(id){
		if($(".validated_form").valid()) {
			...
			$.ajax({
				...
				success:  function(data){
					if(data.status ==1){
						show_info("Data sub kode akuntansi berhasil disimpan!");
	...
	function edit_data(id){
      	$.ajax({
          	type: "GET",
	        url: '{{url("sub_kode_akuntansi")}}/'+id+'/edit',
	        async:true,
	        data: {
	            _token		: "{{csrf_token()}}",
	        },
	        beforeSend: function(data){
		        ...
		        $("#modal-xl .modal-title").html("Edit Data - Sub Kode Akuntansi");
		        $('#modal-xl').modal("show");
