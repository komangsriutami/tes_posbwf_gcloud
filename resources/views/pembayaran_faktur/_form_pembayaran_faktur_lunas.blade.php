...
<script type="text/javascript">
	var token = '{{csrf_token()}}';
 	...
 	var tb_faktur = $('#tb_faktur').DataTable( {
        ...
        serverSide: true,
        ajax: {
            url: '{{url("pembelian/list_pembayaran_faktur_lunas")}}',
		        data:function(d){
   ...
   function export_pembayaran_faktur(){
        window.open("{{ url('pembelian/export') }}"+ "?id_apotek="+$('#id_apotek').val()+"&id_suplier="+$('#id_suplier').val()+"&id_status_lunas=1"+"&tgl_awal="+$('#tgl_awal').val()+"&tgl_akhir="+$('#tgl_akhir').val(),"_blank");
    }
</script>
@endsection