...
@section('content')
	...
	<div class="card card-info card-outline mb-12 border-left-primary">
	    <div class="card-body">
			...
			<a class="btn btn-success w-md m-b-5" href="{{url('agama/create')}}"><i class="fa fa-plus"></i> Tambah Data</a>
	    </div>
	</div>
	...
	<div class="card card-info card-outline" id="main-box" style="">
        ...
        <div class="card-body">
			<table  id="tb_agama" class="table table-bordered table-striped table-hover">
		    	<thead>
			        <tr>
			            <th width="5%">No.</th>
...
<script type="text/javascript">
	...
	var tb_agama = $('#tb_agama').dataTable( {
			processing: true,
	        serverSide: true,
	        stateSave: true,
	        ajax:{
			        url: '{{url("agama/list_agama")}}',
			        data:function(d){
				         }
			     },
	        columns: [
		...
		function(){
			$.ajax({
				...
				success:  function(data){
					if(data==1){
						swal("Deleted!", "Data agama berhasil dihapus.", "success");
					}else{
						
						swal("Failed!", "Gagal menghapus data agama.", "error");
					}
				},
				complete: function(data){
					tb_agama.fnDraw(false);
				},
				error: function(data) {
	...
	function submit_valid(id){
		if($(".validated_form").valid()) {
			...
			$.ajax({
				...
				success:  function(data){
					if(data.status ==1){
						show_info("Data agama berhasil disimpan!");
	...
	function edit_data(id){
      	$.ajax({
	        ...
	        beforeSend: function(data){
		        ...
		        $("#modal-xl .modal-title").html("Edit Data - Agama");
		        $('#modal-xl').modal("show");
