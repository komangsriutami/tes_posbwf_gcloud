    ...
    <div class="card mb-12 border-left-primary card-info">
        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-12">
                    {{$dataTable->table(['id' => 'tb_m_stok_harga_lv'])}}
                </div>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        var editor = new $.fn.dataTable.Editor({
            ajax: "{{ url('/stok_opnam/lavie') }}",
            table: "#tb_m_stok_harga_lv",
            display: "bootstrap",
            fields: [
        ...
        /*$('#tb_m_stok_harga_lv').on('click', 'tbody td:not(:first-child)', function (e) {
            editor.inline(this);
        });*/

        $('#tb_m_stok_harga_lv').on( 'click', 'tbody td.editable', function (e) {
            editor.inline( this );
        });

        editor.field('stok_akhir_so').input().on( 'blur', function (e,d) {
            $('#tb_m_stok_harga_lv_filter label input').focus();
        });
        ...
        $('#so_status_aktif').change(function(){
            $.ajax({
                url:'{{url("stok_opnam/lavie/set_so_status_aktif")}}',
                type: 'POST',
                data: {
                    _token      : "{{csrf_token()}}",
                ...
                success:function(data){
                    //location.reload();
                    window.location.href = "{{ url('stok_opnam/lavie') }}";
                }
            });
        });        
    })
    ...
    function export_data(){
        window.open("{{ url('stok_opnam/lavie/export') }}","_blank");
    }
</script>
@endsection
