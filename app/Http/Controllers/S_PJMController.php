...
use App\DataTables\StokPJMDataTable;
use App\DataTables\StokPJMDataTableEditor;
...
class S_PJMController extends Controller
{
    public function index(StokPJMDataTable $dataTable)
    {
    	$id_apotek = session('id_apotek_active');
    	$now = date('Y-m-d');
    	$cek = SettingStokOpnam::where('id_apotek', $id_apotek)->where('tgl_so', $now)->first();
    	if($id_apotek != 3) {
    		$apotek = MasterApotek::find(3);
    		...
    		if($cek == null) {
    			...
    			return $dataTable->render('so.pjm');
    		}
    	}
    }
    ...
    public function store(StokPJMDataTableEditor $editor)
    {
