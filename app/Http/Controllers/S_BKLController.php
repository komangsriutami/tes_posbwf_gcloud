...
use App\DataTables\StokBKLDataTable;
use App\DataTables\StokBKLDataTableEditor;
...
class S_BKLController extends Controller
{
    public function index(StokBKLDataTable $dataTable)
    {
    	$id_apotek = session('id_apotek_active');
    	$now = date('Y-m-d');
    	$cek = SettingStokOpnam::where('id_apotek', $id_apotek)->where('tgl_so', $now)->first();
    	if($id_apotek != 2) {
    		$apotek = MasterApotek::find(2);
    		...
    		if($cek == null) {
    			...
    			return $dataTable->render('so.bkl');
    		}
    	}
    }
    ...
    public function store(StokBKLDataTableEditor $editor)
    {
