...
use App\DataTables\StokTLDataTable;
use App\DataTables\StokTLDataTableEditor;
...
class S_TLController extends Controller
{
    public function index(StokTLDataTable $dataTable)
    {
    	$id_apotek = session('id_apotek_active');
    	$now = date('Y-m-d');
    	$cek = SettingStokOpnam::where('id_apotek', $id_apotek)->where('tgl_so', $now)->first();
    	if($id_apotek != 5) {
    		$apotek = MasterApotek::find(5);
    ...
    public function store(StokTLDataTableEditor $editor)
    {
