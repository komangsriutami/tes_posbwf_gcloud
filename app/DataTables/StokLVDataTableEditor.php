...
class StokLVDataTableEditor extends DataTablesEditor
{
    ...
    */
    public function updating(Model $model, array $data)
    {
        ...
        if(isset($data['stok_akhir_so'])) {
            ...
            $total_penjualan = DB::table('tb_histori_stok_'.$inisial)->select([DB::raw('SUM(jumlah) as total')])->where('id', $model->id)->where('id_jenis_transaksi', 1)->whereDate('created_at', date('Y-m-d'))->first();
            $total_hapus = DB::table('tb_histori_stok_'.$inisial)->select([DB::raw('SUM(jumlah) as total')])->where('id', $model->id)->where('id_jenis_transaksi', 15)->whereDate('created_at', date('Y-m-d'))->first();
            $total_retur = DB::table('tb_histori_stok_'.$inisial)->select([DB::raw('SUM(jumlah) as total')])->where('id', $model->id)->where('id_jenis_transaksi', 5)->whereDate('created_at', date('Y-m-d'))->first();
            $total_batal_retur = DB::table('tb_histori_stok_'.$inisial)->select([DB::raw('SUM(jumlah) as total')])->where('id', $model->id)->where('id_jenis_transaksi', 6)->whereDate('created_at', date('Y-m-d'))->first();
            ...
            $selisih = $stok_awal_so-($stok_akhir_so+$count_penjualan);
