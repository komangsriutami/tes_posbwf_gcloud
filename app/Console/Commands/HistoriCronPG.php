...
class HistoriCronPG extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'historipg:cron';
    ...
    public function handle()
    {
        ...
        $id_apotek = 4;
        $skip = 0;
        $cek = DB::table('tb_bantu_update_pg')->orderBy('id', 'DESC')->first();
        if(!empty($cek)) {
...
*/
            ...
            if($i > 0) {
                ...
                \Log::info("Cron PG history is working fine! Reload data ".$last_id_obat_ex.' until '.$last_id_obat_after);
            } else {
                \Log::info("Cron PG history not working! Reload data ".$last_id_obat_ex.' until '.$last_id_obat_after);
            }
        } else {
            \Log::info("Cron PG history is working fine! Apotek tidak ditemukan.");
        }
        \Log::info("Cron PG history is working fine!");
    }
}
