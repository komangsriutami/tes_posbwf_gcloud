...
class HistoriCronBKL extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'historibkl:cron';
    ...
    public function handle()
    {
        ...
        $id_apotek = 2;
        $skip = 0;
        $cek = DB::table('tb_bantu_update_bkl')->orderBy('id', 'DESC')->first();
        if(!empty($cek)) {
...
*/
            ...
            if($i > 0) {
                ...
                \Log::info("Cron BKL history is working fine! Reload data ".$last_id_obat_ex.' until '.$last_id_obat_after);
            } else {
                \Log::info("Cron BKL history not working! Reload data ".$last_id_obat_ex.' until '.$last_id_obat_after);
            }
        } else {
            \Log::info("Cron BKL history is working fine! Apotek tidak ditemukan.");
        }
        \Log::info("Cron BKL history is working fine!");
    }
}
