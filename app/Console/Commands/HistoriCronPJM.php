...
class HistoriCronPJM extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'historipjm:cron';
    ...
    public function handle()
    {
        ...
        $id_apotek = 3;
        $skip = 0;
        $cek = DB::table('tb_bantu_update_pjm')->orderBy('id', 'DESC')->first();
        if(!empty($cek)) {
...
*/
            ...
            if($i > 0) {
                ...
                \Log::info("Cron PJM history is working fine! Reload data ".$last_id_obat_ex.' until '.$last_id_obat_after);
            } else {
                \Log::info("Cron PJM history not working! Reload data ".$last_id_obat_ex.' until '.$last_id_obat_after);
            }
        } else {
            \Log::info("Cron PJM history is working fine! Apotek tidak ditemukan.");
        }
        \Log::info("Cron PJM history is working fine!");
    }
}
