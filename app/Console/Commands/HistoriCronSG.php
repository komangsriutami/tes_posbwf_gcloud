...
class HistoriCronSG extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'historisg:cron';
    ...
    public function handle()
    {
        ...
        $id_apotek = 6;
        $skip = 0;
        $cek = DB::table('tb_bantu_update_sg')->orderBy('id', 'DESC')->first();
        if(!empty($cek)) {
...
*/
            ...
            if($i > 0) {
                ...
                \Log::info("Cron SG history is working fine! Reload data ".$last_id_obat_ex.' until '.$last_id_obat_after);
            } else {
                \Log::info("Cron SG SG history not working! Reload data ".$last_id_obat_ex.' until '.$last_id_obat_after);
            }
        } else {
            \Log::info("Cron SG history is working fine! Apotek tidak ditemukan.");
        }
        \Log::info("Cron SG history is working fine!");
    }
}
