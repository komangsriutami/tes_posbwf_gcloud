...
class HistoriCronTL extends Command
{
    ...
    public function handle()
    {
        ...
        $id_apotek = 5;
        $skip = 0;
        $cek = DB::table('tb_bantu_update_tl')->orderBy('id', 'DESC')->first();
        if(!empty($cek)) {
...
*/
            ...
            if($i > 0) {
                ...
                \Log::info("Cron TL history is working fine! Reload data ".$last_id_obat_ex.' until '.$last_id_obat_after);
            } else {
                \Log::info("Cron TL history not working! Reload data ".$last_id_obat_ex.' until '.$last_id_obat_after);
            }
        } else {
            \Log::info("Cron TL history is working fine! Apotek tidak ditemukan.");
        }
        \Log::info("Cron TL history is working fine!");
    }
}
